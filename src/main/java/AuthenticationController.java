...
/**
 * Description: This class handles requests and acts as an interface
 * between the outside world and the AuthenticationService.
 *
 * Intent: The AuthenticationController class serves as a controller for user authentication-related operations.
 * It handles incoming requests, such as user creation, sign-in, password changes,
 * and username updates and deleting users. (CRUD functionality)
 * Upon signing in, the unique user identifier is used as an authentication token.
 * This token will be required to perform authorized actions (updating and deleting users).
